...
function main() {
  ...
  tx(() => {
    ...
    const interest = (
      kind: string,
      at: string,
      ref: { productId?: string; visitorId?: string; customerId?: string },
    ) =>
      run(
        `INSERT INTO interest_events
           (id, business_id, product_id, visitor_id, customer_id, kind, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        newId("int"),
        businessId,
        ref.productId ?? null,
    ...
    for (const visitorId of visitorIds) {
      interest("viewed_shop", iso(Math.floor(random() * 14)), { visitorId });
    }
    ...
    for (let i = 0; i < 5; i++) {
      ...
      interest("asked", at, { productId: product.id, customerId });
    }
    ...
    statuses.forEach((status, index) => {
      ...
      for (const line of unique) {
        ...
        if (status !== "cancelled") {
          interest("ordered", at, { productId: line.id, customerId });
        }
      }
    });
    ...
    for (let i = 0; i < 3; i++) {
      ...
      interest("added_to_cart", at, { productId: product.id, visitorId });
      if (i < 2) interest("checkout_started", at, { visitorId });
    }
  });

  console.log("Seeded Ada's Sneaker Corner.");
  console.log(`  Owner sign-in : ${DEMO_EMAIL} / ${DEMO_PASSWORD}`);
  console.log("  Shop          : /s/adas-sneaker-corner");
}
